...
`verify` 会经由 web 的 nginx 代理与 api 直连分别检查：非法输入返回 422 且定位到字段、不写入记录、锯口计入卷容量、决胜规则输出唯一方案、每卷可逐段复算、方案持久化并可回查；还会验收来源调整关联、裁切进度的顺序完成与末段撤销，以及省略余量与旧行为一致、余量改变分卷、详情闭合和超限不落库；并验收套组编号——无套组请求与旧方案一致、可容纳套组不跨卷且容量复算闭合、不可容纳套组返回定位错误并不落库。
...
| 套组编号 `bundle` | 可选，规则同线段编号（1–32 字符）；相同编号的线段必须落在同一卷 |
...
- 相同套组编号的线段构成不可拆分的组合：同组必须同卷，组内容量 = 各段下料长度合计 + 锯口宽 ×（组内段数 − 1）；未填写的线段仍按原方式独立排卷。
...
实现为子集 DP（≤ 12 段 → 4096 个子集）求最少卷数，再用贪心规范化构造：含最小编号的卷必然排在最前，按字典序枚举其可行段集合并保证剩余段可装入剩余卷数。同一套组的线段在 DP 与规范化构造中作为不可拆分的整体参与（只枚举不拆开任何套组的分法），编号决胜规则不变。结果唯一、确定，不调用任何外部优化服务。`api/tests/test_solver.py` 用暴力枚举所有集合划分做对拍验证。
...
为避免成套信号线被分散到不同线卷，新建（或调整）方案时可给若干线段填写相同的**套组编号**：
...
- 相同套组编号的线段构成不可拆分的组合：求解器把同组线段视为整体，保证它们落在同一卷；组内锯口与各段余量照常计入容量。未填写（或为 `null`）的线段仍按原方式独立排卷。
- 套组只约束「同卷」：最少卷数目标、卷内编号升序、整体字典序决胜规则都不变；交付长度、余量、裁切进度与来源方案关系也不受影响，同组线段仍逐段参与完成与撤销。
- 若某个套组整体（下料长度合计 + 组内锯口）无法装入单卷，返回 **422**，错误定位到该组每条线段的 `bundle` 输入并标明超出的毫米数；页面保留全部输入，失败提交不产生方案：

```json
{
  "detail": [
    {"loc": ["segments", 0, "bundle"], "msg": "bundle 'G9' needs 135 mm (cut lengths + 1 kerfs) and exceeds usable roll length 100 by 35 mm", "type": "value_error"},
    {"loc": ["segments", 1, "bundle"], "msg": "bundle 'G9' needs 135 mm (cut lengths + 1 kerfs) and exceeds usable roll length 100 by 35 mm", "type": "value_error"}
  ]
}
```

- 创建请求、裁切记录与方案详情都保存并返回可空的 `bundle`；详情页在原裁切顺序旁展示套组编号（如 `A（…）［套组 G1］`）。旧客户端省略该字段、历史记录（升级为 `NULL`）均得到原有结果。
...
可空的 `cuts.bundle`（历史记录无套组，排卷行为不变）。
已是最新时为空操作。也可手动执行：
