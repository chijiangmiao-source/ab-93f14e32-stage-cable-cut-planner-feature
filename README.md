...
`verify` 会经由 web 的 nginx 代理与 api 直连分别检查：非法输入返回 422 且定位到字段、不写入记录、锯口计入卷容量、决胜规则输出唯一方案、每卷可逐段复算、方案持久化并可回查；还会验收来源调整关联、裁切进度的顺序完成与末段撤销，以及省略余量与旧行为一致、余量改变分卷、详情闭合和超限不落库。套组相关验收包括：省略套组编号的请求与旧方案一致、可容纳套组不会跨卷且容量复算闭合、不可容纳套组把错误定位到组内每段的套组输入（含超出毫米数）且不落库，以及从页面创建套组方案后刷新详情仍保持相同卷序与套组标识。
...
| 线段套组编号 `kit_no` | 可选，1 至 100000 的整数；填写相同编号的线段构成不可跨卷的一组，缺省/为 `null` 时该段独立排卷 |
...
- 填写相同**套组编号**的线段视为一个不可分割的组合：同组线段必须完整放入同一卷，绝不能跨卷；未填写（或为 `null`）的线段仍按原方式独立排卷。
- 一个套组整体必须能装入单卷：组内各段下料长度合计 + 组内相邻段间的锯口（`组内段数 − 1` 次）≤ 卷长。套组入卷后，它与同卷其他段/组之间仍按普通相邻段各消耗一次锯口。
- 套组只约束分卷归属，不改变交付长度、余量、进度状态或来源方案关系；旧客户端省略 `kit_no` 时，求解结果与历史行为完全一致。
...
实现为子集 DP（≤ 12 段 → 4096 个子集）求最少卷数，再用贪心规范化构造：含最小编号的卷必然排在最前，按字典序枚举其可行段集合并保证剩余段可装入剩余卷数。结果唯一、确定，不调用任何外部优化服务。`api/tests/test_solver.py` 用暴力枚举所有集合划分做对拍验证。存在套组时，先把同组段合并为原子"实体"（无套组时每个实体就是单段，求解结果与旧算法逐案一致），再在实体子集上执行同一套 DP 与贪心，并同样用带套组约束的暴力划分对拍。
...
套组示例：`A`、`B` 同属套组 `7`，求解器保证二者落在同一卷（组内一次锯口照算），`C` 仍独立排卷：
...
该例结果为 `[A, B]`（400 + 300 + 10 = 710，余 190）、`[C]`（500，余 400）；若不填套组，规范解会是 `[A]`、`[B, C]`。

`allowance` 可省略，省略时按 0 处理，结果与旧版本完全一致。`kit_no` 可省略或为 `null`，省略时该段独立排卷，响应中未分组线段与历史记录的 `kit_no` 均为 `null`，结果与旧版本完全一致。方案详情会为每条线段返回交付长度 `length`、余量 `allowance` 与套组编号 `kit_no`，详情页同时展示交付长度、余量、下料长度（两者之和）以及按下料长度复算的每卷余料，并在原裁切顺序旁标明套组编号。
...
（Pydantic 自身的字段错误 `loc` 会带 `"body"` 前缀，前端已兼容。）余量越界（< 0 或 > 10000）由字段校验拒绝；交付长度 + 余量超过卷长时错误定位到对应线段的 `allowance` 输入。套组编号越界（< 1 或 > 100000）由字段校验拒绝；某个套组整体无法装入单卷（计入各段余量与组内锯口后仍超过卷长）时，错误定位到**该组每一条线段的 `kit_no` 输入**，并给出所需长度与超出毫米数；组内若还存在长度/余量非法的线段，则只报对应线段字段、不再重复报套组。前端收到 422 后把错误标回对应输入框，**原输入全部保留**，失败提交不产生任何方案。
